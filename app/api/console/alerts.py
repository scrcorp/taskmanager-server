...
Admin Alert Router — API endpoints for alert management.
...
async def list_alerts(
    ...
    List alerts for the admin user.
...
async def get_unread_count(
    ...
    Get the count of unread alerts for the admin user.
...
async def mark_read(
    ...
    Args:
        alert_id: 알림 UUID 문자열 (Alert UUID string)
        db: 비동기 데이터베이스 세션 (Async database session)
        current_user: 인증된 감독자 이상 사용자 (Authenticated supervisor+ user)
