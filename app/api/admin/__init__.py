...
Included routers (Phase 1 — Foundation):
    ...
    - brands: 브랜드 관리 (Brand management)
    - roles: 역할 관리 (Role management)
    - users: 사용자 관리 (User management)
    - shifts: 근무조 관리 (Shift management under brands)
    - positions: 직책 관리 (Position management under brands)
...
from app.api.admin.brands import router as brands_router
...
admin_router.include_router(brands_router, prefix="/brands", tags=["Brands"])
...
# ---------------------------------------------------------------------------
# 체크리스트: /brands/{brand_id}/checklist-templates 형태 (nested under brands)
