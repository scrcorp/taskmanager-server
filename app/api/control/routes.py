"""Control plane 라우트 — 로그인/로그아웃/대시보드 셸.
...
from app.api.control import pages, ratelimit
from app.api.control import session as cp_session
from app.api.control.deps import COOKIE_NAME, get_current_admin
...
def _base() -> str:
    ...
    return "/" + settings.CONTROL_PLANE_PATH.strip("/")
...
async def login_submit(
    ...
    valid = username == settings.CONTROL_ADMIN_USERNAME and verify_password(
        password, settings.CONTROL_ADMIN_PASSWORD_HASH
    )
    if not valid:
    ...
    resp.set_cookie(
        COOKIE_NAME,
        token,
        max_age=settings.CONTROL_SESSION_MAX_AGE_MINUTES * 60,
        httponly=True,
