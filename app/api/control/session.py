"""Control plane 세션 토큰 — HMAC 서명 쿠키 (외부 의존성 없음).

Stateless signed session for the control plane. No DB, no itsdangerous —
...
def _sign(payload: bytes) -> bytes:
    secret = settings.control_session_secret.encode("utf-8")
    return hmac.new(secret, payload, hashlib.sha256).digest()


def issue_session(username: str) -> str:
    """운영자 username에 대해 서명된 세션 토큰 발급."""
    exp = int(time.time()) + settings.CONTROL_SESSION_MAX_AGE_MINUTES * 60
...
def verify_session(token: str) -> str | None:
    ...
    try:
        ...
        if int(exp_str) < int(time.time()):
            return None
        if username != settings.CONTROL_ADMIN_USERNAME:
            return None
        return username
