...
HEIC 주의: Pillow 기본 빌드는 HEIC를 못 연다(pillow-heif 플러그인 필요). 현재는
HEIC 가 들어오면 디코딩 실패로 pass-through(원본 HEIC 보존) — 기존보다 나빠지지
않음. Hiring HEIC→WebP 변환은 pillow-heif 추가 시 자동 적용(별도 결정).
"""
