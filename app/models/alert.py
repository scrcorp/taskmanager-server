...
Alert SQLAlchemy ORM model definitions.
Implements a polymorphic alert system where each alert
can reference different entity types via reference_type and reference_id.

Tables:
    - alerts: 사용자 알림 (User alerts with polymorphic references)
"""
...
class Alert(Base):
    ...
    Alert model — System alerts delivered to users.
    Uses a polymorphic reference pattern (reference_type + reference_id)
    to link back to the source entity that triggered the alert.

    Alert Types (type 필드 값):
        - "work_assigned": 근무 배정 알림 (New work assignment alert)
        - "additional_task": 추가 업무 알림 (New additional task alert)
        - "notice": 공지사항 알림 (New notice alert)
        - "task_completed": 업무 완료 알림 (Task completion alert)
